...
```SQL
create role admin;
grant all privileges on u_psosnows to adm
```
...
grant execute on delete_user_from_products to platform_director;
```
...
grant select on course_limit_sign to course_coordinator;
...
grant select on financial_reposrt to secretary;
```
...
grant select in studies_meetings_list to lecturer;
```
